class KickBan {
  ...
  async operate ({ client, message, args, config, member, author }, fetch = require("node-fetch"), Database = require("../Models/Member.js"), Schema = require("../Models/Restriction.js")) {
    ...
    if (["kick", "at"].includes(split[0])) {
      ...
      let aprax = message.guild.members.cache.get(target.id);
      if ((!aprax) || (Object.keys(aprax).length <= 2)) return client.tepki(message, "iptal", client.NoMember(message));
      if ((author.roles.highest.position <= aprax.roles.highest.position)) return client.tepki(message, "iptal", client.embed(`${client.react("iptal")} | ${target} kullanıcısı senden üst bir rolde veya aynı roldesiniz, bu işlemi gerçekleştiremem.`, message));
      if (aprax && aprax.bannable) aprax.ban().catch(() => { });
      await client.cezaPuanı(aprax, 30);
      var ceza = await client.cezaNumara();
      var cpoint = await client.cezaPuanCheck(member);
      new Schema({CezaID: ceza, Type: "BAN", userID: member.id, Author: author.id, Reason: reason, DateNow: Date.now(), Activity: true}).save()
      client.SayiPlus(message, Database, "ban");
