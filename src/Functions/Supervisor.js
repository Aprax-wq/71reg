...
async function Welcome (member) {
   ...
   if (sure >= client.getDate(1, "hafta")) {
    ...
    if ((Config.Tags.Tags.some(x => member.user.username.includes(x))) && (Config.Tags.Tags[0] !== "")) member.roles.add(Config.Tags.TagRoles).catch(() => { });
...
${client.react("star")} Tagımızı (\`${Config.Tags.Tags[0]}\`) alarak bizlere destek olabilirsin! Kayıt olmak için teyit odalarına girip ses teyit vermen gerekiyor yetkililerimiz seninle ilgilenecektir! İyi eğlenceler. ${client.react("star")}`, Config.Guild.Welcome);
   } else {
...
async function UserUpdate (member) {
  ...
  if ((Config.Tags.Tags.some(x => Target.user.discriminator.includes(x))) && (!Target.roles.cache.get(Config.Tags.TagRoles))) {
    let Nickname = Target.displayName.replace(Config.Tags.FakeTag, Config.Tags.Tags[0])
  ...
  } else if ((!Config.Tags.Tags.some( x => Target.user.discriminator.includes(x))) && (Target.roles.cache.get(Config.Tags.TagRoles))) {
    let Nickname = Target.displayName.replace(Config.Tags.Tags[0], Config.Tags.FakeTag);
    if ((Target.managable) && (!Target.displayName.inclues(Config.Tags.Tags[0]))) Target.setNickname(Nickname).catch(() => {});
...
async function TagCheck () {
  if (Guild) {
   let unRoles = Config.CezaRoles.filter(x => x != "MuteRoles").map(x => Config.CezaRoles[x])
    ...
    Guild.members.cache.filter(x => (!unRoles.some(v => x.roles.cache.has(v))) && (Config.Tags.Tags.some(aprax => x.user.username.includes(aprax))) && (!x.roles.cache.has(Config.Tags.TagRoles))).forEach((member, index) => {
      setTimeout(() => {
